...
class Button extends JButton {
    ...
    static Button createButton(String type, Color color, String text) {
        if (Objects.equals(type, "bottom")) {
            return new Button(color,text, GUI_old.h1Font);
        }
        else {
            return new Button(color,text, GUI_old.h2Font);
        }
    }
}
...
public class GUI_old {
    ...
    private static GUI_old instance;
    private GUI_old(){
        this.currFrame = mkHomeScreen();
    }

    public static GUI_old getInstance() {
        if (instance == null) {
            instance = new GUI_old();
        }
        return instance;
    }
